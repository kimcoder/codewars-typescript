export class G964 {
    ...
    public static phone = (strng: string, num: string) => {
        let findPhones: string[] = []; 

        strng.split("\n").forEach((str: string) => (str.search(num) > -1) ? findPhones.push(str) : "" );

        switch(findPhones.length) {
            case 0:
                return `Error => Not found: ${num}`;
            case 1:
                let name: string = findPhones[0].match(/(\<)(\D)+(\>)/)![0].replace(/<|>/g, "");
                let address: string = findPhones[0].replace(num, "").replace(/(\<)(\D)+(\>)/, "");
                address = address.replace(/[^a-zA-Z._-\s\d]/g, "").replace(/_|\s{1,}/g, " ");
...
console.log(G964.phone(dr, "1-541-754-3010")); 	// Phone => 1-541-754-3010, Name => J Steeve, Address => 156 Alphand St.
